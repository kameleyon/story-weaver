...
const STYLE_PROMPTS: Record<string, string> = {
  ...
  doodle: `Urban Minimalist Doodle style. LINE WORK: Bold, consistent-weight black outlines (monoline) that feel hand-drawn but clean, with slightly rounded terminals for a friendly, approachable feel. COLOR PALETTE: Muted Primary palette with desaturated tones—dusty reds, sage greens, mustard yellows, and slate blues—set against a warm, textured cream or off-white background reminiscent of recycled paper or newsprint. CHARACTER DESIGN: Object-Head surrealism where a character's head is replaced with a symbolic object, creating an instant iconographic look that is relatable yet stylized. TEXTURING: Subtle Lo-Fi distressing with light paper grain, tiny ink flecks, and occasional print misalignments where the color doesn't perfectly hit the line, giving it a vintage screen-printed quality. COMPOSITION: Centralized and Floating—main subject should be grounded, surrounded by a halo of smaller floating icons (coins, arrows, charts) that represent the theme without cluttering the frame. ART STYLE: Flat 2D Vector Illustration with Indie Comic Aesthetic. VIBE: Lo-fi, Chill, Entrepreneurial, Whimsical. INFLUENCES: Modern editorial illustration, 90s streetwear graphics, and Lofi Girl aesthetics.`,

...
const STYLE_PROMPTS: Record<string, string> = {
  ...
  stick: `Hand-drawn stick figure comic style. CRITICAL BACKGROUND REQUIREMENT: The background MUST be solid pure white (#FFFFFF) with ABSOLUTELY NO horizontal lines, NO notebook paper lines, NO ruled lines, NO faint lines, NO grid patterns, NO paper texture lines—just clean solid white. Crude expressive black marker lines for the drawing only. Extremely simple character designs (circles for heads, single lines for limbs). Strictly black ink on pure white—NO SKIN COLOR, NO FLESH TONES, NO GRAY LINES IN BACKGROUND. Rough sketchy aesthetic similar to 'XKCD' or 'Wait But Why'. Imperfect circles and wobbly lines. High resolution on completely blank white canvas.`,

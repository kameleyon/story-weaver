...
async function generateSceneAudio(
  ...
  if (isHC && customVoiceId && ELEVENLABS_API_KEY) {
    ...
    // Cycle through all keys multiple rounds
...
async function generateImageWithHypereal(
  ...
  try {
    ...
    if (!imageUrl) return { ok: false, error: `No image URL in Hypereal ${mode} response` };
    ...
    console.log(`[HYPEREAL-IMG] ${mode} success: ${bytes.length} bytes`);
...
async function editImageWithReplicatePro(
  ...
  try {
    console.log(`[editImage] Starting image edit with Replicate Nano Banana Pro...`);
    ...
    let fullPrompt = `Edit this image with the following modification: ${editPrompt}
...
STYLE CONTEXT: ${styleDescription}`;
    ...
    // Build input with image_input array for editing
    const input: Record<string, unknown> = {
      prompt: fullPrompt,
      image_input: [sourceImageUrl], // Pass source image URL for editing
      aspect_ratio: aspectRatio,
      output_format: "png",
      resolution: "1K", // Pro model supports 1K resolution
    };

    console.log(`[editImage] Calling Replicate nano-banana-pro with image_input...`);
    ...
    if (!createResponse.ok) {
      const status = createResponse.status;
      const errText = await createResponse.text().catch(() => "");
      console.error(`[editImage] Replicate nano-banana-pro create failed: ${status} - ${errText}`);
      return {
        ok: false,
        error: `Replicate nano-banana-pro edit failed: ${status}${errText ? ` - ${errText}` : ""}`,
      };
    }

    let prediction = await createResponse.json();
    console.log(`[editImage] Nano Banana Pro prediction started: ${prediction.id}, status: ${prediction.status}`);
    ...
    if (prediction.status === "failed") {
      console.error(`[editImage] Nano Banana Pro prediction failed: ${prediction.error}`);
    ...
    if (!imageUrl) {
      console.error(
        `[editImage] Nano Banana Pro no image URL in response:`,
        JSON.stringify(prediction.output).substring(0, 200),
      );
      return { ok: false, error: "No image URL returned from Nano Banana Pro edit" };
    }

    console.log(`[editImage] Nano Banana Pro edit success, downloading from: ${imageUrl.substring(0, 80)}...`);

    const imgResponse = await fetch(imageUrl);
    if (!imgResponse.ok) return { ok: false, error: "Failed to download edited image" };

    const bytes = new Uint8Array(await imgResponse.arrayBuffer());
    console.log(`[editImage] Nano Banana Pro edit completed: ${bytes.length} bytes`);
    return { ok: true, bytes };
  } catch (err) {
...
OUTPUT: Ultra high resolution, professional illustration with dynamic composition, clear visual hierarchy, cinematic quality, bold creativity, and meticulous attention to detail.`;
  ...
  await logSystemEvent({
    ...
    details: {
      ...
      model: useProModel ? "nano-banana-pro" : "nano-banana",
      format,
      style,
    },
    generationId,
  ...
  // Track images generated for cost logging
...
async function handleFinalizePhase(
  ...
  // Calculate image cost based on Replicate pricing
  const useProModel = costTracking.imageModel?.includes("pro");
  const imageCost = costTracking.imagesGenerated * (useProModel ? PRICING.imageNanoBananaPro : PRICING.imageNanoBanana);
  ...
  const { error: costError } = await supabase.from("generation_costs").insert({
    ...
    replicate_cost: imageCost + audioCost, // Replicate handles images + audio
    hypereal_cost: 0, // Hypereal removed from pipeline
  ...
  if (costError) {
    ...
    console.log(
      `[FINALIZE] Cost recorded: $${costTracking.estimatedCostUsd.toFixed(4)} (Replicate: $${imageCost.toFixed(4)})`,
    );
  }
...
Professional illustration with dynamic composition and clear visual hierarchy.`;
    ...
    console.log(
      `[regenerate-image] Scene ${sceneIndex + 1}, Image ${targetImageIndex + 1} - Apply Edit`,
    );
    ...
    let fullEditPrompt = `Edit this image with the following modification: ${imageModification}
...
- Maintain the same artistic style and color palette`;
    ...
    if (scene.title || scene.subtitle) {
      fullEditPrompt += `\n\nTEXT OVERLAY TO PRESERVE:`;
      if (scene.title) fullEditPrompt += `\n- Title: "${scene.title}"`;
      if (scene.subtitle) fullEditPrompt += `\n- Subtitle: "${scene.subtitle}"`;
      fullEditPrompt += `\nEnsure these text elements remain visible and properly positioned in the modified image.`;
    }
    ...
    // Log the API call with actual provider used
    await logApiCall({
      supabase,
      userId: user.id,
      generationId,
    ...
    if (!imageResult.ok) {
      ...
      await logSystemEvent({
        supabase,
        userId: user.id,
        eventType: "img2img_edit_fallback",
        category: "system_warning",
        message: `img2img edit failed on all providers, falling back to text-to-image generation`,
        details: {
          sceneIndex,
          targetImageIndex,
          error: editError,
...
Professional illustration with dynamic composition and clear visual hierarchy. Apply the user's modification to enhance the image while maintaining consistency with the original scene concept.`;
      ...
      // Try Hypereal T2I first for fallback, then Replicate
      const fallbackStartTime = Date.now();
      let fallbackProvider = "replicate";
      let fallbackModel = "google/nano-banana-pro";

      if (hyperealKey) {
        console.log(`[regenerate-image] Fallback: Trying Hypereal T2I`);
        const hrFallback = await generateImageWithHypereal(modifiedPrompt, format, hyperealKey, true);
        if (hrFallback.ok) {
          ...
          console.warn(`[regenerate-image] Fallback: Hypereal T2I failed: ${hrFallback.error}, using Replicate`);

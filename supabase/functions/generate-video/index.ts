...
const STYLE_PROMPTS: Record<string, string> = {
  ...
  stick: `STRICTLY BLACK AND WHITE ONLY. Hand-drawn stick figure comic style. Crude, expressive black marker lines on a pure white or notebook paper background. Extremely simple character designs (circles for heads, single lines for limbs). ABSOLUTELY NO FILL COLORS, NO SKIN TONES, NO COLOR OF ANY KIND—only black lines on white paper. Black and white line art ONLY. Focus on humor and clarity. Rough, sketchy aesthetic similar to XKCD or Wait But Why. Imperfect circles and wobbly lines to emphasize the handmade, napkin-sketch quality. NO COLORS WHATSOEVER—monochrome black ink on white background only.`,

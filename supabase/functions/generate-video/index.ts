...
const STYLE_PROMPTS: Record<string, string> = {
  ...
  realistic: `Ultra-realistic DSLR portrait photo, Canon EOS 5D Mark III look, 85mm prime lens, medium close-up. Natural skin texture, sharp eyes, subtle stubble detail, realistic hair strands. Shallow depth of field with creamy bokeh, background softly blurred. Lighting: soft key light from a large window camera-left (diffused), gentle fill from reflector camera-right, subtle rim light from behind to separate shoulders/hair, warm practical lamp in background for depth. Color grade: clean cinematic neutral tones, slightly cool shadows, natural warm skin, high dynamic range (HDR/UDR), crisp but not oversharpened. Resolution: UHD 4K, high detail, natural film-like grain. Shot on Canon 5D Mark III, 85mm, f/1.8–2.2, 1/160s, ISO 100–400, white balance 5200K. No cartoon, no CGI, no 3D render, no plastic skin, no beauty-filter, no airbrushed, no over-smoothed face, no oversharpened, no uncanny valley, no distorted eyes, no extra fingers, no text, no watermark, no low-res, no noisy.`,
